...
type Challenge = {
  ...
  description: string;
  root_cause: string;
  fix_action_key: string;
  hint_ladder: string[];
  fix_options: { key: string; label: string }[];
};

export function scenarioFor(slug: string): Scenario | undefined {
  return SCENARIOS[slug];
}

export async function getChallenge(slug: string): Promise<Challenge> {
  const rows = await query<Challenge>("SELECT * FROM challenges WHERE slug = $1", [slug]);
  if (!rows[0]) throw new Error(`Unknown challenge: ${slug}`);
  return rows[0];
}
...
export async function getChallengeById(id: string): Promise<Challenge> {
  ...
  if (!rows[0]) throw new Error(`Unknown challenge id: ${id}`);
  return rows[0];
}

export async function listChallenges(): Promise<Challenge[]> {
  return query<Challenge>("SELECT * FROM challenges ORDER BY difficulty, title");
}

export async function startIncident(userId: string, challengeSlug: string) {
  ...
  const rows = await query<{ id: string }>(
    `INSERT INTO incidents (user_id, challenge_id, status) VALUES ($1, $2, 'active') RETURNING id`,
    [userId, challenge.id]
  );
...
async function logEvent(incidentId: string, eventType: string, payload: unknown) {
...
export async function getIncidentSummary(incidentId: string) {
  const state = await redis.hgetall(keys.state(incidentId));
  if (!state.startedAtMs) throw new Error("Incident not found");
...
export async function getMetrics(incidentId: string) {
  const state = await redis.hgetall(keys.state(incidentId));
  if (!state.startedAtMs) throw new Error("Incident not found");
  const scenario = scenarioFor(state.challengeSlug)!;
  const elapsed = elapsedSeconds(Number(state.startedAtMs));
...
export async function runCommand(incidentId: string, command: string) {
  const state = await redis.hgetall(keys.state(incidentId));
  if (!state.startedAtMs) throw new Error("Incident not found");
  const scenario = scenarioFor(state.challengeSlug)!;
  const elapsed = elapsedSeconds(Number(state.startedAtMs));
...
export async function getHint(incidentId: string) {
  const state = await redis.hgetall(keys.state(incidentId));
  if (!state.startedAtMs) throw new Error("Incident not found");
  const challenge = await getChallenge(state.challengeSlug);
...
export async function submitFix(incidentId: string, actionKey: string) {
  const state = await redis.hgetall(keys.state(incidentId));
  if (!state.startedAtMs) throw new Error("Incident not found");
  ...
  await query(
    `INSERT INTO scores (incident_id, diagnosis_score, time_seconds, hints_used, fix_quality, overall_score)
     VALUES ($1,$2,$3,$4,$5,$6)
    ...
    [incidentId, score.diagnosisScore, timeSeconds, hintsUsed, score.fixQuality, score.overall]
  );
...
export async function getReplay(incidentId: string) {
  const events = await query(
    `SELECT event_type, payload, occurred_at FROM incident_events WHERE incident_id = $1 ORDER BY occurred_at ASC`,
    [incidentId]
  );
