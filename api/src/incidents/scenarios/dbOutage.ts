...
function metricsAt(elapsed: number): MetricTick {
...
function serviceGraph(broken: boolean): ServiceNode[] {
  return [
...
export const dbOutageScenario: Scenario = {
  slug: "db-outage",
  ...
  commands: {
    ...
    "describe service payment-api": ({ broken }) =>
      broken
    ...
    help: () =>
      ["available commands:", "curl /api/health", "logs tail", "describe service payment-api", "show config", "netstat"].join(
        "\n"
      ),
  },
};
