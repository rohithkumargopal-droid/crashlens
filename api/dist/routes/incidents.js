...
import { startIncident, getIncidentSummary, getLogs, getMetrics, runCommand, getHint, submitFix, getReplay, listChallenges, } from "../incidents/engine.js";
...
incidentsRouter.post("/incidents/start", async (req, res) => {
    try {
        const { challengeSlug, displayName } = req.body;
        let userId = req.body.userId;
        if (!userId) {
            const rows = await query("INSERT INTO users (display_name) VALUES ($1) RETURNING id", [displayName || `player-${uuid().slice(0, 6)}`]);
            userId = rows[0].id;
        }
...
incidentsRouter.post("/incidents/:id/fix", async (req, res) => {
    try {
        ...
        if (result.correct && !("alreadyResolved" in result)) {
            ...
            if (userId) {
                await redis.zadd(keys.leaderboard(), result.score.overall, userId);
            }
        }
        res.json(result);
    }
